...
export const NotificationItem: React.FC<NotificationItemProps> = ({ notification, onClick }) => {
    return (
        <button
            ...
            className={`w-full text-left p-3 flex items-start gap-3 transition-colors ${!notification.isRead ? 'bg-slate-800/50' : ''} hover:bg-slate-700/50 rounded-lg`}
        >
            ...
            <div className="flex-grow">
                ...
                <p className="text-xs text-slate-400 mt-1">{timeSince(new Date(notification.timestamp))}</p>
            </div>
        </button>
    );
};
